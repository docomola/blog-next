...
````
  ...
  <TabItem label="渲染效果" defaultOpen>
...
- **參數：**
  | 組件 | 參數名稱 | 類型 | 說明 |
  | --- | --- | --- | --- |
  | `<Tabs />` | `class` | string | 額外的 CSS Class，可以用來自定義分頁組件的樣式。 | |
  | `<TabItem />` | `label` | string | 分頁的標籤名稱，將顯示在分頁標籤上。 |
  | `<TabItem />` | `class` | string | 額外的 CSS Class，可以用來自定義分頁項目的樣式。 |

你可以使用 `Tabs` 和 `TabItem` 組件來創建一個分頁組件，這對於展示多個相關內容非常有用。
...
如果你在同一個`<Tabs>`組件中同時使用了`defaultOpen`屬性，系統將會跳出錯誤，如下範例所示。
